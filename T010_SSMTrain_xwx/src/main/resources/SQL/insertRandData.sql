...
BEGIN
  ...
  DECLARE firstnammes VARCHAR(1000);
  DECLARE lastnames VARCHAR(1000);
  DECLARE sex CHAR(1);
  DECLARE score CHAR(2);
  DECLARE sql_part VARCHAR(100) DEFAULT "insert into t_user(first_name,last_name,sex,score,copy_id) values ";
  SET firstnammes =
  ...
  WHILE rowid < num DO
  #截取姓氏字符串中的一个字符
  SET firstname = SUBSTRING(firstnammes, CEILING((length(firstnammes)-1) * RAND()), 1);
  #截取名字字符串中的一个或两个字符
  SET lastname = CONCAT(SUBSTRING(lastnames, CEILING((length(lastnames)-1) * RAND()), 1),
                        SUBSTRING(lastnames, CEILING((length(lastnames)-1) * RAND()), ROUND(Rand())));
