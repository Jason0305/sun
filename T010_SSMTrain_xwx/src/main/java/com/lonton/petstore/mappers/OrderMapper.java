...
public interface OrderMapper {
    ...
    @Insert({
        ...
        "#{recvAddress,jdbcType=VARCHAR}, #{total,jdbcType=DECIMAL}, ",
    ...
    @Select({
        "select",
        "id, uid, recv_name, recv_phone, recv_address, total, status, created_time, pay_time",
        "from t_order",
        "where id = #{id,jdbcType=INTEGER}"
    })
    @ConstructorArgs({
        ...
        @Arg(column="total", javaType=BigDecimal.class, jdbcType=JdbcType.DECIMAL),
